/* CopyTrader dashboard — live client.
 * Connects to /ws (WebSocket) for a 2s live state feed, falls back to
 * /api/state polling if WebSocket fails.
 */
(() => {
  "use strict";
  ...
  function render(s) {
    state = s;
    const meta = s.meta, pf = s.portfolio;

    // badges
    const mode = $("badge-mode");
    mode.textContent = meta.mode;
    mode.className = "badge " + (meta.mode === "live" ? "badge-live" : "badge-paper");
    const api = $("badge-api");
    api.textContent = meta.api_ok ? "API canlı" : "API bağlantı hatası";
    api.className = "badge " + (meta.api_ok ? "badge-ok" : "badge-warn");
    const ai = $("badge-ai");
    ai.textContent = meta.ai_enabled ? "AI aktif" : "AI kapalı";
    ai.className = "badge " + (meta.ai_enabled ? "badge-on" : "badge-off");

    // stats
    $("stat-equity").textContent = fmtUsd(pf.equity);
    const ed = $("stat-equity-delta");
    const total = pf.realized_pnl + pf.unrealized_pnl;
    ed.textContent = (total >= 0 ? "+" : "") + fmtUsd(total);
    ed.className = "delta " + cls(total);
    $("stat-realized").textContent = fmtUsd(pf.realized_pnl);
    $("stat-realized").className = "val " + cls(pf.realized_pnl);
    $("stat-unrealized").textContent = fmtUsd(pf.unrealized_pnl);
    $("stat-unrealized").className = "val " + cls(pf.unrealized_pnl);
    $("stat-open").textContent = pf.open_positions;
    $("stat-funding").textContent = fmtUsd(pf.total_funding_collected);
    $("stat-scans").textContent = meta.scan_count;
    ...
    if (s.market.rows && s.market.rows.length) {
      mb.innerHTML = s.market.rows.map(r => {
        ...
        return `<tr>
          ...
          <td class="${cls(fr)}">${fr === null ? "—" : fr.toFixed(4) + "%"}</td>
          <td>${vol}</td>
        </tr>`;
      }).join("");
    ...
    if (s.positions.length) {
      pb.innerHTML = s.positions.map(p => {
        ...
        return `<tr>
          ...
          <td>${fmtUsd(p.funding_collected || 0)}</td>
          <td><button class="btn btn-ghost" onclick="window.closePos(${p.id})">Kapat</button></td>
        </tr>`;
      }).join("");
    } else {
    ...
    if (s.latest_signals && s.latest_signals.length) {
      sl.innerHTML = s.latest_signals.slice().reverse().map(sg =>
        `<div class="signal">
          <span class="sym">${sg.symbol}</span>
          <span class="side-${sg.side.toLowerCase()}">${sg.side}</span>
          <span class="rsn">${sg.reason}</span>
          <span class="muted">${(sg.confidence * 100).toFixed(0)}%</span>
        </div>`).join("");
    } else {
    ...
    if (!equityChart) {
      equityChart = new Chart(canvas, {
        ...
        options: {
          ...
          scales: {
            x: { ticks: { color: "#8b8b9e", maxTicksLimit: 8 }, grid: { color: "#1a1a24" } },
  ...
  window.closePos = async (id) => {
    ...
    if (res.ok) alert("Pozisyon kapatıldı — PnL: $" + res.pnl.toFixed(2));
    else alert("Kapatılamadı: " + (res.error || "bilinmeyen hata"));
  };
  ...
  // start: settings first, then live feed
